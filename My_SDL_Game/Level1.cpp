...
Level1::Level1(DungeonCrawlerGame* pOwner, PlayerCharacter* pPlayer, CollisionManager* pCollisionManager,
               TTF_Font* pFont)
	:m_pOwner(pOwner)
	,mpPlayer(pPlayer)
...
void Level1::Enter()
{
	...
	{
		// Map
		mGameObjects.push_back(new TileMap(this, mpCollisionManager, "assets/Levels/Room1.txt"));
	...
	{
		mGameObjects.push_back(new DumbGhost(Vector2<float>{350, 259}, Vector2<float>{32, 32}, mpPlayer, mpCollisionManager, speed2));
		speed2 += 20;
	}
...
void Level1::Update(double deltaTime)
{
	...
	{
		m_pOwner->LoadScene(DungeonCrawlerGame::Room2Screen);
	}
...
bool Level1::HandleEvent(SDL_Event* pEvent)
{
